...
###########################
simulate = True  # Set to False to run on OPX, True to simulate
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot results
        # ax[0].suptitle(f"Qubit spectroscopy - LO = {qubit_LO / u.GHz} GHz")
        # ax[0].plot(211)
        ax[0].cla()
        ax[0].plot((dfs + center) / u.MHz, R, ".")
        ax[0].set_xlabel("Qubit intermediate frequency [MHz]")
        ax[0].set_ylabel(r"$R=\sqrt{I^2 + Q^2}$ [V]")
        ax[1].cla()

        ax[1].plot((dfs + center) / u.MHz, phase, ".")
        ax[1].set_xlabel("Qubit intermediate frequency [MHz]")
