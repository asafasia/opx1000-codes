...
# from quam_config.wiring_lffem_mwfem import machine
...
rr_freq = np.array([7.61]) * u.GHz
rr_LO = 7.5 * u.GHz
rr_if = (
    rr_freq - rr_LO
)  # The intermediate frequency is inferred from the LO and readout frequencies
...
for k, qubit in enumerate(machine.qubits.values()):
    qubit.resonator.f_01 = rr_freq.tolist()[
        k
    ]  # Resonator frequency optimized for discriminating 0 (|g>) and 1 (|e>)
    qubit.resonator.RF_frequency = qubit.resonator.f_01  # Readout frequency
    qubit.resonator.opx_output.full_scale_power_dbm = (
        rr_full_scale  # Max readout power in dBm
    )
    qubit.resonator.opx_output.upconverter_frequency = (
        rr_LO  # Readout up-converter frequency
    )
    qubit.resonator.opx_input.band = get_band(
        rr_LO
    )  # Readout band for the up-conversion
    qubit.resonator.opx_output.band = get_band(
        rr_LO
    )  # Readout band for the down-conversion
...
xy_if = (
    xy_freq - xy_LO
)  # The intermediate frequency is inferred from the LO and qubit frequencies
...
for k, qubit in enumerate(machine.qubits.values()):
    ...
    qubit.xy.opx_output.upconverter_frequency = xy_LO.tolist()[
        k
    ]  # Qubit drive up-converter frequency
    qubit.xy.opx_output.band = get_band(
        xy_LO.tolist()[k]
    )  # Qubit drive band for the up-conversion
...
for k, q in enumerate(machine.qubits):
    ...
    # Qubit saturation
    machine.qubits[q].xy.operations["x180"] = const_pulse
    machine.qubits[q].xy.operations["x180"].length = const_pulse.length

    machine.qubits[q].xy.operations["saturation"].length = 50 * u.us
    machine.qubits[q].xy.operations["saturation"].amplitude = 0.1 * xy_amplitude

    SquarePulse(length=20, amplitude=1)
    # machine.qubits[q].xy.operations["x180"].length = 40 * u.ns
    # machine.qubits[q].xy.operations["x180"].amplitude = xy_amplitude
########################################################################################################################
