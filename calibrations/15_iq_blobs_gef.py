"""Class-based calibration for 15_iq_blobs_gef."""
...
class IqBlobsGef(BaseCalibration[Parameters, Quam]):
    """Class-based calibration for ``calibrations/15_iq_blobs_gef.py``."""
        ...
        with program() as node.namespace["qua_program"]:
            ...
            for multiplexed_qubits in qubits.batch():
                ...
                with for_(n, 0, n < n_runs, n + 1):
                    ...
                    for i, qubit in multiplexed_qubits.items():
                        qubit.wait(
                            2 * qubit.thermalization_time * u.ns
                        )  # longer wait for |f> thermalization
                    align()
                    # |g> state readout
                    for i, qubit in multiplexed_qubits.items():
        ...
        with node.record_state_updates():
            for q in node.namespace["qubits"]:
                ...
                node.machine.qubits[q.name].resonator.gef_centers = (
                    node.results["ds_fit"].sel(qubit=q.name).center_matrix.data
                    * operation.length
                    / 2**12
