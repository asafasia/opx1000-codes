...
def _extract_relevant_fit_parameters(ds_fit: xr.Dataset, node: QualibrationNode):
    ...
    for qubit in node.namespace["qubits"]:
        ...
        if qubit_name not in ds_fit.qubit.values:
            logging.warning(f"Qubit {qubit_name} not found in the fit results.")
            continue

        fit_results[qubit_name] = FitParameters(
            optimal_detuning=ds_fit.optimal_detuning.sel(qubit=qubit_name).item(),
            success=ds_fit.success.sel(qubit=qubit_name).item(),
        )
