...
@dataclass
class CpmgFit:
    """Fitted CPMG coherence values for one qubit."""

    t2_cpmg: dict[int, float]
    t2_cpmg_error: dict[int, float]
...
def _fit_one_trace(
    ...
    try:
        ...
        if log_callable is not None:
            ...
            n_pi = trace.coords.get("n_pi", "?")
            log_callable(f"CPMG decay fit failed for qubit={qubit}, n_pi={n_pi}: {error}")
...
def _fit_all(data: xr.DataArray, ds: xr.Dataset, log_callable=None) -> xr.DataArray:
    ...
    for qubit in data.qubit.values:
        qubit_fits = []
        for n_pi in data.n_pi.values:
            trace = data.sel(qubit=qubit, n_pi=n_pi)
            fit = _fit_one_trace(trace, ds.evolution_time, log_callable=log_callable)
            qubit_fits.append(fit)
        fits.append(xr.concat(qubit_fits, dim=xr.IndexVariable("n_pi", data.n_pi.values)))
...
def _fit_r_squared(data: xr.DataArray, fit: xr.DataArray, ds: xr.Dataset) -> xr.DataArray:
    results = []
    for n_pi in data.n_pi.values:
        selected_fit = fit.sel(n_pi=n_pi)
        fitted = decay_exp(
            ds.evolution_time,
        ...
        selected_data = data.sel(n_pi=n_pi)
    ...
    return xr.concat(results, dim=xr.IndexVariable("n_pi", data.n_pi.values))


def fit_raw_data(ds: xr.Dataset, node: QualibrationNode) -> Tuple[xr.Dataset, dict[str, CpmgFit]]:
...
def _extract_relevant_fit_parameters(fit: xr.Dataset):
    t2_cpmg = -1 / fit.fit_data.sel(fit_vals="decay")
    t2_cpmg_error = -t2_cpmg * (
        np.sqrt(fit.fit_data.sel(fit_vals="decay_decay")) / fit.fit_data.sel(fit_vals="decay")
    )
    success = (
        np.isfinite(t2_cpmg)
        & np.isfinite(t2_cpmg_error)
        & (t2_cpmg > 16)
        & (t2_cpmg_error >= 0)
        & (t2_cpmg_error / t2_cpmg < 1)
    )

    fit = fit.assign(
        {
            "T2_cpmg": t2_cpmg,
            "T2_cpmg_error": t2_cpmg_error,
            "success": success,
        }
    )
    fit.T2_cpmg.attrs = {"long_name": "T2 CPMG", "units": "ns"}
    fit.T2_cpmg_error.attrs = {"long_name": "T2 CPMG error", "units": "ns"}

    fit_results = {}
    for qubit in fit.qubit.values:
        selected = fit.sel(qubit=qubit)
        fit_results[str(qubit)] = CpmgFit(
            t2_cpmg={
                int(n): float(selected.T2_cpmg.sel(n_pi=n))
                for n in fit.n_pi.values
            },
            t2_cpmg_error={
                int(n): float(selected.T2_cpmg_error.sel(n_pi=n))
                for n in fit.n_pi.values
            },
            success={
                int(n): bool(selected.success.sel(n_pi=n))
                for n in fit.n_pi.values
            },
        )
...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for qubit, result in fit_results.items():
        for n_pi, t2 in result["t2_cpmg"].items() if isinstance(result, dict) else result.t2_cpmg.items():
            if isinstance(result, dict):
                error = result["t2_cpmg_error"][n_pi]
                success = result["success"][n_pi]
            else:
                error = result.t2_cpmg_error[n_pi]
                success = result.success[n_pi]
            status = "SUCCESS" if success else "FAIL"
            log_callable(
                f"CPMG T2 for {qubit}, N={n_pi}: {1e-3 * t2:.2f} +/- {1e-3 * error:.2f} us --> {status}"
            )
