"""Class-based v2 migration for resonator_parameter_scan."""
...
Runs a resonator spectroscopy grid in one OPX job, while saving and analyzing
every parameter combination separately.
"""
...
def analyze_and_save(dataset: xr.Dataset, output_dir: Path) -> pd.DataFrame:
    ...
    for amp_factor in READOUT_AMPLITUDE_FACTORS:
        for q_offset in QUBIT_OFFSETS_HZ:
            ...
            row = {
                ...
                "apparent_resonator_shift_hz": float(rr_freq[driven_idx] - rr_freq[ground_idx]),
    ...
    strongest["apparent_qubit_frequency_hz"] = base_q + strongest["qubit_frequency_offset_hz"]
...
class ResonatorParameterScan(BaseCalibration[NodeParameters, Quam]):
    """v2 class migration for ``calibrations/resonator_parameter_scan.py``."""
    ...
    def execute_qua_program(self) -> None:
        ...
        self.results["ds_raw"].attrs.update(
