...
try:
    from .fwhm import add_gaussian_fwhm_analysis
except ImportError:  # Support direct file loading in focused tests.
    from Projects.shaped_pulse_spectroscopy.shaped_pulse_spectroscopy.fwhm import (
        add_gaussian_fwhm_analysis,
    )
...
PROJECT_PULSE_AMPLITUDE_CEILING_V = 1
...
def amplitude_prefactors(parameters) -> np.ndarray:
    """Return the Lorentzian amplitude prefactors for a sweep."""
    spacing = getattr(parameters, "amp_factor_spacing", "linear")
    points = getattr(parameters, "amp_factor_points", None)
    if points is not None:
        ...
        if spacing == "log":
            if parameters.min_amp_factor <= 0 or parameters.max_amp_factor <= 0:
                raise ValueError(
                    "Log-spaced amplitude prefactors require positive min/max factors."
                )
            return np.geomspace(
    ...
    if spacing != "linear":
        raise ValueError("amp_factor_spacing requires amp_factor_points.")
...
def _pulse_metadata(parameters, namespace: dict | None = None) -> dict[str, object]:
    ...
    metadata: dict[str, object] = {
        ...
        "amp_factor_points": getattr(parameters, "amp_factor_points", None),
        "amp_factor_spacing": getattr(parameters, "amp_factor_spacing", None),
...
def _parameter_lines(ds: xr.Dataset, qubits: List[AnyTransmon]) -> list[str]:
    ...
    for qubit in qubits:
        ...
        coherence_parts = [
            ...
            _format_seconds(_t2_star_seconds(qubit), "T2*"),
            _format_hz(_t2_star_limit_hz(qubit), "1/(pi*T2*)"),
        ]
...
def _t2_star_limit_hz(qubit: AnyTransmon) -> float | None:
    t2_star_s = _t2_star_seconds(qubit)
    return None if t2_star_s is None else 1 / (np.pi * t2_star_s)


def _t2_limit_hz(qubit: AnyTransmon) -> float | None:
    """Compatibility alias for the Ramsey T2* FWHM limit."""
    return _t2_star_limit_hz(qubit)
...
def _t2_seconds(qubit: AnyTransmon) -> float | None:
    """Compatibility alias for Ramsey T2*."""
    return _t2_star_seconds(qubit)


def _add_t2_star_coordinates(
    ds: xr.Dataset,
    qubits: List[AnyTransmon],
) -> xr.Dataset:
    """Persist the Ramsey T2* and its FWHM limit with each inner dataset."""
    t2_star_s = np.array(
        [
            value if (value := _t2_star_seconds(qubit)) is not None else np.nan
            for qubit in qubits
        ],
        dtype=float,
    )
    t2_star_limit_hz = np.where(
        np.isfinite(t2_star_s) & (t2_star_s > 0),
        1 / (np.pi * t2_star_s),
        np.nan,
    )
    ds = ds.assign_coords(
        t2_star_s=("qubit", t2_star_s),
        t2_star_fwhm_limit_hz=("qubit", t2_star_limit_hz),
    )
...
def _add_t2_limit_lines(ax, qubit: AnyTransmon) -> None:
    ...
    limit_mhz = 1 / (2 * np.pi * t2_star_s) / u.MHz
...
def _add_fwhm_markers(ax, selected: xr.Dataset) -> None:
    if "rabi_frequency_MHz" not in selected.variables:
        return
...
def _scatter_fwhm_edges(
    ...
    required = {f"{prefix}_fwhm_left_hz", f"{prefix}_fwhm_right_hz"}
    if not required.issubset(set(selected.variables)):
        return
    left = np.asarray(selected[f"{prefix}_fwhm_left_hz"].values, dtype=float) / u.MHz
    right = np.asarray(selected[f"{prefix}_fwhm_right_hz"].values, dtype=float) / u.MHz
    for x_values, marker in ((left, "<"), (right, ">")):
        ...
        ax.scatter(
            ...
            edgecolors=color,
            linewidths=1.1,
            label=label if marker == "<" else None,
            zorder=5,
        )
...
def _add_single_amp_fwhm_lines(ax, selected: xr.Dataset) -> None:
    for prefix, label, color in (
        ...
        for name, edge_label in (
            ...
            (f"{prefix}_fwhm_right_hz", None),
        ):
            if name not in selected.variables:
                continue
